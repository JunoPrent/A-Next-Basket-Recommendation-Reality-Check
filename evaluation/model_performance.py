...
def get_repeat_eval(pred_folder, dataset, size, fold_list, file, use_attention, use_item_order):
    ...
    for ind in ast.literal_eval(fold_list):
        ...
        pred_file = f'{pred_folder}/{dataset}_{attention_str}{use_item_order_str}pred{ind}.json'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datasets', nargs='+', default=["dunnhumby", "tafeng", "instacart", "instacart_itemorder"], help='List with datasets, choice from dunnhumby, tafeng, instacart, instacart_itemorder')
    ...
    eval_file = f'eval_{datasets_str}_{use_attention_str}{use_item_order_str}results.txt'
    # print("The adjusted version is being executed")
    f = open(eval_file, 'w')
    for dataset in datasets:
        f.write('############'+dataset+'########### \n')
        get_repeat_eval(pred_folder, dataset, 10, fold_list, f, use_attention, use_item_order)
        get_repeat_eval(pred_folder, dataset, 20, fold_list, f, use_attention, use_item_order)
        # future work: test on small basket sizes for determining temporal effect of add-order
        # get_repeat_eval(pred_folder, dataset, 1, fold_list, f, use_attention, use_item_order)
        # get_repeat_eval(pred_folder, dataset, 5, fold_list, f, use_attention, use_item_order)
